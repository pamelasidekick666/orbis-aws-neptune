...
edge_columns = ["~id", "id", "~label", "~from", "~to",
                "start_date:String", "end_date:String", "direct_ownership:Double", "total_ownership:Double", "weight:Int", "relationship:String"]
node_columns = ["~id", "id", "~label", "name:String",
                       "gender:String", "birthdate:String", "incorporation_date:String", "bvd_id:String", "uci:String", "is_manager_type:String", "entity_type:String", "first_name:String", "last_name:String", "address:String",
...
def add_edge(from_id, to_id, relationship, tracker, direct=0.0, total=0.0, weight=0):
    global edge_df
    new_row = {
        ...
        "end_date:Date": None,
        "direct_ownership:Double": direct,
        "total_ownership:Double": total,
        "weight:Int": weight
    }
...
def extract_phone(result, prefix, from_id, tracker):
    ...
    new_row = {
        ...
        "name:String": result[prefix + 'PHONE_NUMBER'],
        "country_iso:String": "",
        "city:String": "",
        "postcode:String": "",
        ...
        "bvd_id:String": "",
        "address:String": "",
        "phone:String": result[prefix + 'PHONE_NUMBER'],
        "email:String": "",
        "entity_type:String": "",
...
def extract_entities(result, prefix, tracker):
    ...
    # Main
    if prefix == "":
        if result["INCORPORATION_DATE"] is not None:
            incorporation_date = datetime.strptime(
                result["INCORPORATION_DATE"], "%Y-%m-%dT%H:%M:%S")
        ...
        new_row = {
            ...
            "phone:String": result['PHONE_NUMBER'] if result['PHONE_NUMBER'] is not None else "",
        ...
        if result["BO_UCI"] is not None:
            if result["BO_BIRTHDATE"] is not None:
                birthdate = result["BO_BIRTHDATE"].strftime('%Y-%m-%d')
            else:
                birthdate = result["BO_BIRTHDATE"]
            new_row = {
                "~id": "v" + str(tracker.vertex_id),
                "id": tracker.vertex_id,
                "~label": "Person",
...
def process_shareholder_bo(result, tracker):
    ...
    for i in range(len(sh)):
        tracker.vertex_id = 1
        add_edge("v" + str(tracker.main_vertex_id), "v" + str(tracker.vertex_id), "Shareholder", tracker, float(sh[i]["SH_DIRECT_PCT"]) if sh[i]["SH_DIRECT_PCT"] != "n.a." else 0.0, float(
            sh[i]["SH_TOTAL_PCT"]) if sh[i]["SH_TOTAL_PCT"] != "n.a." else 0.0)
